// src/components/books/BookSearchInput.tsx
...
export default function BookSearchInput({ onSelect, placeholder }: Props) {
  ...
  async function handleSelect(book: ExternalBook) {
    ...
    // Enriquece com BrasilAPI se tiver ISBN (metadados pt-BR + capa melhor)
  ...
  return (
    <div ref={wrapRef} className="book-search">
      <div className="book-search__field">
        ...
        {(loading || enriching) && (
          <span className="spinner book-search__loading" />
        )}
      </div>
      ...
      {error && !open && (
        <p
          ...
          style={{ marginTop: 'var(--space-1)', color: 'var(--color-muted)' }}
        >
          {error}
        </p>
      )}
      ...
      {open && results.length > 0 && (
        <div className="book-search__dropdown" role="listbox">
          {results.map((b) => (
            <button
            ...
            >
              <div
                ...
                style={{
                  backgroundImage: b.coverUrl ? `url(${b.coverUrl})` : undefined,
                }}
              >
                {!b.coverUrl && (
                  <svg
